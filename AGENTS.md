...
**`src/lib/agents.ts` + `src/lib/acp.ts` + `src/lib/codexModels.ts`** — Harness spawn/lifecycle and ACP sessions. Agents run as child processes with JSON-RPC over stdout; tool calls and reasoning chunks parse into structured events. Codex uses the pinned maintained ACP adapter with the user-selected executable passed as `CODEX_PATH`; probes source models, reasoning, and Fast support from that same executable. Runs persist through app restarts via native blob storage.
