...
/**
 * ACP's standard method-not-found rejection is JSON-RPC `-32601`, but not all
 * harnesses report unsupported extension methods that way. oh-my-pi 16.3.11
 * rejects `session/set_model` as `-32603` with
 * `data.details = "Unknown ACP ext method: session/set_model"`, while still
 * supporting the equivalent `session/set_config_option("model", …)` path. Treat
 * both forms as method-not-found so callers can fall back without masking real
 * handler failures.
 */
export function isMethodNotFound(e: unknown): boolean {
 if (!(e instanceof AcpRpcError)) return false;
 if (e.rpcCode === -32601) return true;

 let detail = e.message;
 if (e.rpcData !== undefined) {
  try {
   detail += ` ${JSON.stringify(e.rpcData)}`;
  } catch {
   detail += ` ${String(e.rpcData)}`;
  }
 }

 return /unknown (acp )?(ext )?method|unsupported (acp )?(ext )?method|method not found/i.test(
  detail,
 );
}
