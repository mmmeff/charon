...
/** git merge-base --is-ancestor returns 0 (yes), 1 (no), or >1 (error). */
async function isAncestor(wt: Worktree, ancestor: string, descendant: string): Promise<boolean> {
 const res = await native.runGit(["merge-base", "--is-ancestor", ancestor, descendant], wt.path);
 if (res.code === 0) return true;
 if (res.code === 1) return false;
 throw new Error(
  `git merge-base --is-ancestor ${ancestor.slice(0, 7)} ${descendant.slice(0, 7)} failed (${res.code}):\n${res.stderr || res.stdout
  }`
 );
}
 ...
 * app's own push path). We detect that case via ancestry: if the remote
 * advanced from the worktree's starting commit and the remote tip is reachable
 * from local HEAD the agent pushed it (intermediate or final) — fail the run
 * rather than blessing an unvalidated push. If the remote moved but is NOT
 * reachable from HEAD, a concurrent run moved it and the rebase+retry path
 * handles it.
 *
...
export async function validateAndPush(ctx: FlowContext, runId: string, wt: Worktree): Promise<string | null> {
 ...
 try {
  ...
  // Agent-side push detection: if the remote advanced from wt.baseSha and is
  // reachable from local HEAD, the agent pushed it. merge-base --is-ancestor
  // exits 0 (ancestor) or 1 (not ancestor) without error. If remote is NOT an
  // ancestor of HEAD, a concurrent run moved it — the rebase+retry path below
  // handles that case cleanly.
  if (remote !== wt.baseSha) {
   if (await isAncestor(wt, remote, head)) {
    const detail = remote === head
     ? `The commit ${head.slice(0, 7)} is already on the remote.`
     : `Remote tip ${remote.slice(0, 7)} (pushed by the agent, then superceded by local commits) is already part of the branch history.`;
    throw new Error(
     `Agent pushed to origin/${wt.prBranch} bypassing Charon's validation gate. ${detail} ` +
     `Review the push manually; CI is the only check it got.`
    );
   }
  }
