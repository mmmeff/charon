...
// app mode → ordered ACP session-mode preference. Ask falls back to plan on
// harnesses like oh-my-pi that only expose default/plan: the permission
// auto-approve below relies on ACP read-only modes preventing writes.
const ACP_MODE_PREFS: Record<string, string[]> = {
 write: ["agent"],
 plan: ["plan"],
 ask: ["ask", "plan"],
};
...
export async function startAgent(opts: StartAgentOptions): Promise<string> {
 ...
 void (async () => {
  try {
   ...
   const modes = ns.modes;
   const modePrefs = ACP_MODE_PREFS[opts.mode ?? "write"] ?? [];
   const targetMode = modes
    ? modePrefs.find((modeId) =>
   ...
   //   Keep `set_config_option` skipped unless `set_model` is rejected as a
   //   missing method — that was the actual corrupting call — but `set_model`
   //   is safe and now required for opencode.
   //
   //   oh-my-pi is the inverse case: verified firsthand on omp 16.3.11, it
   //   exposes models only through a `model` config option, has no
   //   `session/set_model` handler, and accepts
   //   `session/set_config_option("model", …)`. Method-not-found rejections
