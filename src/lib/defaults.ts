...
export const HARNESS_MODEL_DEFAULTS: Record<string, HarnessModelDefaults> = {
 ...
 // oh-my-pi exposes models as a `model` config option (provider-prefixed ids)
 // plus a separate `thinking` axis (off/auto/…/xhigh). Opus for
 // judgment-heavy review/Q&A, sonnet for prose, the fable flagship for the
 // rest. "auto" thinking defers the per-prompt effort choice to the harness.
 omp: {
  defaultModel: "anthropic/claude-fable-5",
  modelOverrides: {
   review: "anthropic/claude-opus-4-8",
   draft_question: "anthropic/claude-opus-4-8",
   rewrite: "anthropic/claude-sonnet-4-6",
  },
